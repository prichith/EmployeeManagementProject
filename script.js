...
//EMPLOYEES PAGE
    ...
    //BUTTONS AND FUNCTIONS
        addEmployeeOpenBtn.addEventListener("click",function(){
    ...
    const optionBoxEl = (actions)=>{
        actions.classList.toggle("pops");
        // actions.classList.contains("pops")? actions.classList.remove("pops") : actions.classList.add("pops")
...
// DISPLAY EMPLOYEES IN TABLE BODY
    function displayEmployees(value,sl){
        ...
        <td><img src="http://localhost:3000/employees/${value.id}/avatar" alt=profilepic class="rounded-circle mr-2 mx-2" height=30  width=30/>${value.salutation}${value.firstName} ${value.lastName}</td>
        <td>${value.email}</td>
        <td>${value.phone}</td>
        <td>${value.gender}</td>
        <td>${value.dob}</td>
        ...
        <td>
            ...
            <div id="optionBox${index}" class="optionBox">
                <a href="employeeDetails.html?id=${value.id}" target="blank"><button onclick="viewDetails('${value.id}')" class="optionBoxBtn"><span class="material-symbols-outlined">visibility</span><p>View Details</p></button></a>
                <button onclick="editDetails('${value.id}'); " class="optionBoxBtn"><span class="material-symbols-outlined">edit</span><p>Edit</p></button>
                <button onclick="deleteDetails('${value.id}')" class="optionBoxBtn"><span class="material-symbols-outlined">delete</span><p>Delete </p></button>
            </div>
        </td>
        </tr>`
        // First number of SL
...
// PAGINATION SECTION
    ...
    let start; //global variable for sl number
    let end; //global
        ...
        function pagination(objectData){
            ...
            // ONCLICK FUNCTION OF THE PAGES
                function employeesPerPage(pageNum){
                    let employeeArray=[]
                    let EP = ((pageNum-1)*employeeCount)-1
                    start = EP + 1 
                    end = start+(employeeCount-1)
...
overlayOptionBox.addEventListener("click",function(){
